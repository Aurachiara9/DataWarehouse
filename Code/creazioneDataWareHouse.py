...
# OrderItemInformation.csv 
unique_item_id = pd.DataFrame({'order_item_id': df_base['order_item_id'].unique()})
unique_item_id['ID_order_item'] = np.random.choice(np.arange(1, 1 + len(unique_item_id)), len(unique_item_id), replace=False)
unique_item_id = unique_item_id[['ID_order_item', 'order_item_id']]
unique_item_id.to_csv(os.path.join(OUTPUT_PATH, 'OrderItemInformation.csv'), index=False)
...
fact_table = fact_table.merge(unique_item_id, on='order_item_id', how='left')
...
pk_columns = [
    ...
    'ID_order_approved_at', 'ID_order_delivered_carrier_date', 'ID_order_item'
]
